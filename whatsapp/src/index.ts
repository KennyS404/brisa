...
// Função para lidar com múltiplas mensagens
// Função para lidar com múltiplas mensagens
const handleIncomingMessages = async (messages: Array<any>, sock: any) => {
  // Cria uma lista de promessas para processar cada mensagem
  const messagePromises = messages.map(async (msg) => {
    ...
    if (!prompt || !senderId || !pushName || !messageTimestamp) {
      console.error("Dados incompletos na mensagem:", msg);
      return;
    }
    ...
    try {
      ...
      await sendMessageWTyping(sock, { text: message }, senderId!);
    } catch (error) {
...
const startSock = async () => {
  ...
  if (useMobile && !sock.authState.creds.registered) {
    ...
    const mcc =
      PHONENUMBER_MCC[
        phoneNumber.countryCallingCode as keyof typeof PHONENUMBER_MCC
      ];
    if (!mcc) {
      throw new Error(
    ...
    async (events: any) => {
      ...
      if (events["messages.upsert"]) {
        ...
        if (upsert.type === "notify") {
          // Usa a função para lidar com todas as mensagens de uma vez
          await handleIncomingMessages(upsert.messages, sock);
        }
      }
...
if (cluster.isMaster) {
  ...
  // Apenas um worker deve iniciar a conexão com o WhatsApp
